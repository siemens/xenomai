...
extern pthread_key_t __xeno_tskey;
...

{
    ...
    err = XENOMAI_SKINCALL2(__xeno_muxid,
			    __xeno_task_create,
			    &bulk,
			    iargs->completionp);
    if (err)
...
{
    return XENOMAI_SKINCALL0(__xeno_muxid,
			     __xeno_task_yield);
}
...
{
    int err = XENOMAI_SKINCALL3(__xeno_muxid,
                                __xeno_task_set_periodic,
                                task,
                                &idate,
                                &period);
    if(err == -ETIMEDOUT)
...

{
    ...
    self = (RT_TASK *)pthread_getspecific(__xeno_tskey);
    ...
    if (!self || XENOMAI_SKINCALL1(__xeno_muxid,__xeno_task_self,self) != 0)
	return NULL;

    pthread_setspecific(__xeno_tskey,self);
...
ssize_t rt_task_send (RT_TASK *task,
		      ...
		      RTIME timeout)
{
    return (ssize_t)XENOMAI_SKINCALL4(__xeno_muxid,
				      __xeno_task_send,
				      task,
				      mcb_s,
				      mcb_r,
