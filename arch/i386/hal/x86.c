...

{
    ...
    printk(KERN_INFO "Xenomai: hal/x86 loaded.\n");
}
...
#ifdef CONFIG_XENO_HW_X86_SEP
    if (!cpu_has_sep)
        printk("Xenomai: SEP enabled for syscalls but CPU has no SEP support,\n"
               "         Disabling it would be slightly better performance-wise.\n");
#endif /* CONFIG_XENO_HW_X86_SEP */
...
{
    rthal_smi_restore();
    printk(KERN_INFO "Xenomai: hal/x86 unloaded.\n");
}
