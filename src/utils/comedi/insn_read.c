...
int main(int argc, char *argv[])
{
    ...
    while((ret = getopt_long(argc,
			     argv,
			     "vrd:s:S:c:R:h", insn_read_opts, NULL))>=0) {
	switch(ret) {
	case 'v':
	    verbose = 1;
	    break;
	...
	if (ret < 0) {
	    fprintf(stderr, "insn_read: rt_task_shadow failed (ret=%d)\n",
		    ret);
	    goto out_insn_read;
	}
	...
	ret = comedi_get_rnginfo(&dsc, idx_subd, idx_chan, idx_rng, &rnginfo);
	if(ret < 0) {
	    fprintf(stderr, 
	...
	for(i = 0; i < ret; i++) {
	    ...
	    */
		...
		if(idx_rng >=0)
		    printf("\t-> %F", dbuf[i / (chinfo->nb_bits / 8)]);
