...
STATUS taskActivate(TASK_ID task_id)
{
    ...
    err = XENOMAI_SKINCALL1(__vxworks_muxid, __vxworks_task_activate, task_id);
    if (err) {
        errno = abs(err);
        return ERROR;
    }
...
STATUS taskSuspend(TASK_ID task_id)
{
    ...
    err = XENOMAI_SKINCALL1(__vxworks_muxid, __vxworks_task_suspend, task_id);
    if (err) {
        errno = abs(err);
        return ERROR;
    }
...
STATUS taskResume(TASK_ID task_id)
{
    ...
    err = XENOMAI_SKINCALL1(__vxworks_muxid, __vxworks_task_resume, task_id);
    if (err) {
        errno = abs(err);
        return ERROR;
    }
