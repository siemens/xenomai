		      ...
		      xncompletion_t *completionp, void **khandlep)
{
	...
	return XENOMAI_SKINCALL4(__uvm_muxid,
				 __uvm_thread_create,
				 name, khandlep, uhandle, completionp);
}
...
{
	return XENOMAI_SKINCALL1(__uvm_muxid, __uvm_thread_start, khandle);
}
...
{
	return XENOMAI_SKINCALL1(__uvm_muxid, __uvm_thread_idle, lockp);
}

int uvm_thread_cancel(void *deadhandle, void *nexthandle)
{
	return XENOMAI_SKINCALL2(__uvm_muxid,
				 __uvm_thread_cancel, deadhandle, nexthandle);
}
...
{
	return XENOMAI_SKINCALL1(__uvm_muxid, __uvm_timer_read, tp);
}
...
int uvm_timer_tsc(nanotime_t *tp)
{
	...
	*tp = __xn_rdtsc();
	...
	return XENOMAI_SKINCALL1(__uvm_muxid, __uvm_timer_tsc, tp);
