...
document] instead. Please note that Xenomai 2.x has reached EOL status
and is no more actively maintained.
...
This requires special kernel support to deliver fast and deterministic
response time to external interrupts.  Such support is provided by the
interrupt pipeline (aka I-pipe) in the form of a kernel patch you have
to apply against a mainline kernel tree. Then, the _Cobalt_ codebase
implementing real-time services can be compiled in the patched kernel.

Preparing the _Cobalt_ kernel
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The first step is to build _Cobalt_ as part of the target kernel. To
this end, `scripts/prepare-kernel.sh` is a shell script which sets up
