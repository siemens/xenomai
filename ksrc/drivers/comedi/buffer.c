...
void comedi_free_buffer(comedi_buf_t * buf_desc)
{
  ...
  if(buf_desc->buf!=NULL)
  {
    ...
    for (vaddr = vabase; vaddr < vabase + buf_desc->size; vaddr += PAGE_SIZE)
...
int comedi_alloc_buffer(comedi_buf_t * buf_desc)
{
  ...
  for (vaddr = vabase; vaddr < vabase + buf_desc->size; vaddr += PAGE_SIZE)
    buf_desc->pg_list[(vaddr-vabase)>>PAGE_SHIFT] = __va_to_kva(vaddr);
...
int comedi_ioctl_mmap(comedi_cxt_t *cxt, void *arg)
{
    ...
    if(comedi_copy_from_user(cxt, 
			     &map_cfg, 
			     arg, sizeof(comedi_mmap_t))!=0)
	return -EFAULT;

    /* Checks the subdevice */
    if(map_cfg.idx_subd >= dev->transfer->nb_subd ||
       (dev->transfer->subds[map_cfg.idx_subd]->flags & COMEDI_SUBD_CMD) == 0 ||
       (dev->transfer->subds[map_cfg.idx_subd]->flags & COMEDI_SUBD_MMAP) == 0)
	return -EINVAL;
...
int comedi_ioctl_bufinfo(comedi_cxt_t *cxt, void *arg)
{
    ...
    if((dev->transfer->subds[info.idx_subd]->flags & 
	COMEDI_SUBD_CMD) == 0)
	return -EINVAL;
...
ssize_t comedi_write(comedi_cxt_t *cxt, const void *bufdata, size_t nbytes)
{
	...
	if(tmp_cnt > 0) {
	    ...
	    /* Performs the copy */
	    ret = __produce(cxt, buf, (void *)bufdata + count, tmp_cnt);
	    if(ret < 0) {
		count = ret;
		goto out_comedi_write;
	    }
...
int comedi_ioctl_poll(comedi_cxt_t *cxt, void *arg)
{
    ...
    if(!test_bit(COMEDI_TSF_BUSY, 
		 &(dev->transfer->status[poll.idx_subd])))
	return -EINVAL;
	...
	ret = comedi_timedwait_sync(&(buf->sync), 
				    comedi_test_rt(), ns);
    }
