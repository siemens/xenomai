...
int comedi_proc_attach(comedi_cxt_t *cxt)
{
    ...
    {
	comedi_logerr("comedi_proc_attach: failed to allocate buffer\n");
	return -ENOMEM;
    }
    ...
    if(entry==NULL)
    {
	comedi_logerr("comedi_proc_attach: failed to create /proc/comedi/%s\n",
	       entry_name);
	ret=-ENOMEM;
...
void comedi_proc_detach(comedi_cxt_t *cxt)
{
    ...
    if((p=entry_name)==NULL)
    {
	comedi_logerr("comedi_proc_detach: failed to allocate filename buffer\n");
	return;
    }
			...
			void *arg)
{
    ...
    if(ret != 0) {
	comedi_logerr("comedi_fill_lnkdesc: call1(copy_from_user) failed\n");
	...
	if(tmpname == NULL) {
	    comedi_logerr("comedi_fill_lnkdesc: call1(alloc) failed\n");    
	    ret = -ENOMEM;
	    goto out_get_lnkdesc;      
	}
	...
	ret = comedi_copy_from_user(cxt, 
				    tmpname, 
				    link_arg->bname, link_arg->bname_size);
	if(ret != 0) {
	    comedi_logerr("comedi_fill_lnkdesc: call2(copy_from_user) failed\n");    
	    goto out_get_lnkdesc;
	}
    }
    else {
	...
	if(tmpopts == NULL) {
	    comedi_logerr("comedi_fill_lnkdesc: call2(alloc) failed\n");          
	    ret = -ENOMEM;
	    goto out_get_lnkdesc;      
	}
	...
	ret = comedi_copy_from_user(cxt,
				    tmpopts, 
				    link_arg->opts, link_arg->opts_size);
	if(ret != 0) {
	    comedi_logerr("comedi_fill_lnkdesc: call3(copy_from_user) failed\n");      
	    goto out_get_lnkdesc;    
	}
    }
...
void comedi_free_lnkdesc(comedi_cxt_t *cxt, 
			 comedi_lnkdesc_t *link_arg)
{
			 ...
			 comedi_lnkdesc_t *link_arg)
{
	...
	comedi_loginfo("comedi_assign_driver: warning! the field priv will not be usable\n");
    else {
	dev->priv = comedi_kmalloc(drv->privdata_size);
	if(dev->priv == NULL && drv->privdata_size != 0) {
	    comedi_logerr("comedi_assign_driver: call(alloc) failed\n");
	    ret = -ENOMEM;
	...
	comedi_logerr("comedi_assign_driver: call(drv->attach) failed (ret=%d)\n",ret);  
...
int comedi_ioctl_devcfg(comedi_cxt_t *cxt, void *arg)
{
    ...
    if(arg == NULL) {
	/* Basic checking */
	if(!test_bit(COMEDI_DEV_ATTACHED, &(comedi_get_dev(cxt)->flags)))
	    return -EINVAL;
	...
	if((ret = comedi_device_detach(cxt)) == 0)
	    clear_bit(COMEDI_DEV_ATTACHED, &(comedi_get_dev(cxt)->flags));
    }
    else {
	/* Basic checking */
	if(test_bit(COMEDI_DEV_ATTACHED, &(comedi_get_dev(cxt)->flags)))
	...
	if((ret = comedi_setup_transfer(cxt)) != 0 || 
	   (ret = comedi_proc_attach(cxt)) != 0)
	    ...
	    set_bit(COMEDI_DEV_ATTACHED, &(comedi_get_dev(cxt)->flags));
    }
