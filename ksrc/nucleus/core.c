...

{
    ...
    if (nkpod == &__xeno_pod && countq(&nkpod->threadq) == 0)
	{
	xncore_umount();
	return 1;
	}
...

{
    ...
    if (xnpod_init(&__xeno_pod,XNCORE_MIN_PRIO,XNCORE_MAX_PRIO,0) != 0)
	return -ENOSYS;

    __xeno_pod.svctable.unload = &xncore_unload_hook;
    __xeno_pod.refcnt = 1;
