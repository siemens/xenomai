...
const char *xnpod_fatal_helper(const char *format, ...)
{
        ...
        while (holder) {
            ...
            if (thread->status & XNINVPS)
                snprintf(pbuf, sizeof(pbuf), "%3d(%d)",
                         thread->cprio, xnpod_rescale_prio(thread->cprio));
            else
    ...
    if (testbits(nkpod->status, XNTIMED))
        ...
        p += snprintf(p, XNPOD_FATAL_BUFSZ - (p - nkmsgbuf), "Timer: none\n");
  out:
    ...
    if (xnpod_shadow_p()) {
        ...
        if (xnarch_fault_notify(fltinfo))   /* Don't report debug traps */
            xnprintf("Switching %s to secondary mode after exception #%u from "
                     ...
                     thread->name,
                     xnarch_fault_trap(fltinfo),
                     xnarch_fault_pc(fltinfo), xnthread_user_pid(thread));
...
int xnpod_init(xnpod_t *pod, int minpri, int maxpri, xnflags_t flags)
{
    ...
    for (cpu = 0; cpu < nr_cpus; ++cpu) {
        ...
        err = xnthread_init(&sched->rootcb,
                            root_name, XNPOD_ROOT_PRIO_BASE, XNROOT | XNSTARTED
        ...
        xnarch_init_root_tcb(xnthread_archtcb(&sched->rootcb),
                             &sched->rootcb, xnthread_name(&sched->rootcb));
                                       ...
                                       xnthread_t *threadin)
{
    ...
    if (!emptyq_p(&nkpod->tdeleteq) && !testbits(threadout->status, XNROOT)) {
                       ...
                       void (*entry) (void *cookie), void *cookie)
{
    ...
    __setbits(thread->status,
              (mode & (XNTHREAD_MODE_BITS | XNSUSP)) | XNSTARTED);
    thread->imask = imask;
    ...
    if (!emptyq_p(&nkpod->tstartq) && !testbits(thread->status, XNROOT)) {
...
void xnpod_delete_thread(xnthread_t *thread)
{
    ...
    if (sched->runthread == thread) {
        ...
        if (!emptyq_p(&nkpod->tdeleteq) && !testbits(thread->status, XNROOT)) {
                          ...
                          xnflags_t mask, xnticks_t timeout, xnsynch_t *wchan)
{
    ...
    else if (testbits(thread->status, XNSHADOW | XNRELAX) ==
...
void xnpod_dispatch_signals(void)
{
    ...
    if (thread->signals == 0 ||
        testbits(thread->status, XNASDI) || thread->asr == XNTHREAD_INVALID_ASR)
        return;
...
void xnpod_schedule(void)
{
  ...
  do_switch:
    ...
    if (!emptyq_p(&nkpod->tswitchq) && !testbits(runthread->status, XNROOT)) {
...
void xnpod_schedule_runnable(xnthread_t *thread, int flags)
{
    ...
    if (thread != runthread) {
        ...
        if (!testbits(runthread->status, XNTHREAD_BLOCK_BITS | XNREADY)) {
            ...
            if (testbits(runthread->status, XNLOCK))
                ...
                sched_insertpql(&sched->readyq, &runthread->rlink,
                                runthread->cprio);
  ...
  maybe_switch:
    ...
    xnarch_switch_to(xnthread_archtcb(runthread), xnthread_archtcb(threadin));
...
void xnpod_stop_timer(void)
{
    ...
    if (!nkpod ||
        testbits(nkpod->status, XNPIDLE) || !testbits(nkpod->status, XNTIMED)) {
        xnlock_put_irqrestore(&nklock, s);
        return;
    }
...
int xnpod_announce_tick(xnintr_t *intr)
{
    ...
    if (xnarch_get_cpu_tsc() >= sched->watchdog_trigger) {
        if (!xnpod_root_p() && sched->watchdog_armed) {
            ...
            xnpod_suspend_thread(xnpod_current_thread(), XNSUSP, XN_INFINITE,
                                 NULL);
        } else {
            xnpod_reset_watchdog(sched);
        }
