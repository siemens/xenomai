...
void xnltt_log_mark(const char *fmt, ...)
{
    ...
    if (xnltt_evtable[xeno_ev_mark].ltt_filter & xnltt_filter) {
        ...
        va_end(ap);
        ltt_log_std_formatted_event(xnltt_evtable[xeno_ev_mark].ltt_evid,
                                    markbuf);
    }
}
...
struct xnltt_evmap xnltt_evtable[] = {
    ...
    [xeno_ev_fault] =
        {"Xenomai fault", "thread=%s, location=%p, trap=%d", -1, xeno_evall},
    [xeno_ev_callout] =
    ...
    [xeno_ev_thrsuspend] =
        {"Xenomai thread suspend",
         "thread=%s, mask=0x%x, timeout=%Lu, wchan=%p", -1, xeno_evthr},
    [xeno_ev_thresume] =
        {"Xenomai thread resume", "thread=%s, mask=0x%x", -1, xeno_evthr},
    [xeno_ev_thrunblock] =
        {"Xenomai thread unblock", "thread=%s, status=0x%x", -1, xeno_evthr},
    [xeno_ev_threnice] =
    ...
    [xeno_ev_shadowmap] =
        {"Xenomai shadow map", "thread=%s, pid=%d, prio=%d", -1, xeno_evthr},
    [xeno_ev_shadowunmap] =
    ...
    [xeno_ev_thrsetmode] =
        {"Xenomai thread setmode", "thread=%s, clrmask=0x%x, setmask=0x%x", -1,
         xeno_evthr},
    [xeno_ev_rdrotate] =
