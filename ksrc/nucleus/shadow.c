...
static int get_system_info (struct task_struct *curr,
			    int muxid, /* unused */
			    u_long infarg)
{
    xnsysinfo_t info;
...

{
    ...
    /* If we are kicking a shadow thread, make sure Linux won't
       ...
       signal_wake_up(). The Xenomai scheduler must remain in control
       for now, until we explicitely relax the shadow thread to allow
       for processing the pending signals. Make sure we keep the
