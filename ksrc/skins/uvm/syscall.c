...
static inline xnthread_t *__vm_find_thread(struct task_struct *curr,
					   void *khandle)
{
	xnthread_t *thread;
			      ...
			      xncompletion_t __user * u_completion)
{
	...
	if (__xn_reg_arg2(regs) &&
	    !__xn_access_ok(curr, VERIFY_WRITE, __xn_reg_arg2(regs),
			    sizeof(thread)))
		return -EFAULT;
...
{
	if (!__xn_access_ok
	    (curr, VERIFY_WRITE, __xn_reg_arg4(regs), sizeof(xncompletion_t)))
		return -EFAULT;

	return __vm_shadow_helper(curr, regs,
				  (xncompletion_t __user *)__xn_reg_arg4(regs));
}
...
static int __vm_timer_read(struct task_struct *curr, struct pt_regs *regs)
{
	...
	t = xnpod_get_time();
	__xn_copy_to_user(curr, (void __user *)__xn_reg_arg1(regs), &t,
			  sizeof(t));
