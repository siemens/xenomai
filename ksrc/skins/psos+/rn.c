...
int psosrn_init(u_long rn0size)
{
    ...
    rn_create("RN#0", rn0addr, rn0size, 128, RN_FORCEDEL, &rn0id, &allocsize);
...
u_long rn_ident(char name[4], u_long *rnid)
{
    ...
    for (holder = getheadq(&psosrnq); holder; holder = nextq(&psosrnq, holder)) {
...
u_long rn_retseg(u_long rnid, void *chunk)
{
    ...
    if (xnsynch_nsleepers(synch) > 0) {
        ...
        while ((holder = nholder) != NULL) {
            ...
            chunk = xnheap_alloc(&rn->heapbase, sleeper->waitargs.region.size);
            if (chunk) {
