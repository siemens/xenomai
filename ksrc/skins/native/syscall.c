                            ...
                            unsigned magic, void **objaddrp)
{
    ...
    __xn_strncpy_from_user(curr, name, (const char __user *)__xn_reg_arg2(regs),
                           sizeof(name) - 1);
...
static int __rt_task_wait_period(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (__xn_reg_arg1(regs) && (err == 0 || err == -ETIMEDOUT))
        __xn_copy_to_user(curr, (void __user *)__xn_reg_arg1(regs), &overruns,
                          sizeof(overruns));
...
static int __rt_task_inquire(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!err)
        __xn_copy_to_user(curr, (void __user *)__xn_reg_arg2(regs), &info,
                          sizeof(info));
...
static int __rt_task_set_mode(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (__xn_reg_arg3(regs))
        __xn_copy_to_user(curr, (void __user *)__xn_reg_arg3(regs), &mode_r,
                          sizeof(mode_r));
...
static int __rt_task_send(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (__xn_reg_arg3(regs)) {
        ...
        __xn_copy_from_user(curr, &mcb_r, (void __user *)__xn_reg_arg3(regs),
                            sizeof(mcb_r));
    ...
    if (__xn_reg_arg3(regs)) {
        mcb_r.data = data_r;
        __xn_copy_to_user(curr, (void __user *)__xn_reg_arg3(regs), &mcb_r,
                          sizeof(mcb_r));
    }
...
static int __rt_timer_inquire(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!err)
        __xn_copy_to_user(curr, (void __user *)__xn_reg_arg1(regs), &info,
                          sizeof(info));
...
static int __rt_queue_bind(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    err =
        __rt_bind_helper(curr, regs, &ph.opaque, XENO_QUEUE_MAGIC, (void **)&q);
...
static int __rt_heap_free(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (buf) {
        buf =
            xnheap_mapped_address(&heap->heap_base, (caddr_t) buf - ph.mapbase);
...
static int __rt_pipe_write(struct task_struct *curr, struct pt_regs *regs)
{
    ...
    __xn_copy_from_user(curr, P_MSGPTR(msg), (void __user *)__xn_reg_arg2(regs),
                        size);
