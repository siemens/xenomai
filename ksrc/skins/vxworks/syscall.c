...

{
	...
	__xn_strncpy_from_user(curr,name,(const char __user *)bulk.a1,sizeof(name) - 1);
	...
	*name = '\0';
	...
	__xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&ph,sizeof(ph));
...
static int __wind_task_deleteforce (struct task_struct *curr, struct pt_regs *regs)
{
...
static int __wind_task_priorityset (struct task_struct *curr, struct pt_regs *regs)
{
    xnhandle_t handle = __xn_reg_arg1(regs);
...
static int __wind_task_priorityget (struct task_struct *curr, struct pt_regs *regs)
{
    xnhandle_t handle = __xn_reg_arg1(regs);
    WIND_TCB *pTcb;
    int prio;

    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(prio)))
	return -EFAULT;
    ...
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&prio,sizeof(prio));
...
static int __wind_sem_mcreate (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(sem_id)))
	return -EFAULT;
...
static int __wind_taskinfo_name (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),name,strlen(name) + 1);
...
static int __wind_taskinfo_iddfl (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(ret_id)))
	return -EFAULT;

    ret_id = taskIdDefault(handle);
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&ret_id,sizeof(ret_id));
...
static int __wind_taskinfo_status (struct task_struct *curr, struct pt_regs *regs)
{
    xnhandle_t handle = __xn_reg_arg1(regs);
    ...
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&status,sizeof(status));
...
static int __wind_errno_taskget (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(errcode)))
	return -EFAULT;
    ...
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&errcode,sizeof(errcode));
...
static int __wind_kernel_timeslice (struct task_struct *curr, struct pt_regs *regs)
{
    int ticks = __xn_reg_arg1(regs);
...
static int __wind_msgq_nummsgs (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(nummsgs)))
	return -EFAULT;
    ...
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&nummsgs,sizeof(nummsgs));
...
static int __wind_msgq_receive (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg5(regs),sizeof(nbytes)))
	return -EFAULT;
    ...
    if (err != ERROR)
	{
	__xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),msgbuf,nbytes);
	__xn_copy_to_user(curr,(void __user *)__xn_reg_arg5(regs),&err,sizeof(err));
	err = 0;
	}
    else
	err = wind_errnoget();
...
static int __wind_msgq_send (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    __xn_copy_from_user(curr,msgbuf,(void __user *)__xn_reg_arg2(regs),nbytes);
...
static int __wind_sys_clkrateget (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg1(regs),sizeof(hz)))
	return -EFAULT;

    hz = sysClkRateGet();
    __xn_copy_to_user(curr,(void __user *)__xn_reg_arg1(regs),&hz,sizeof(hz));
...

{
    ...
    if (wdStart((WDOG_ID)wd,timeout,(wind_timer_t)&__wind_wd_handler,(long)wd) == ERROR)
	{
	xnlock_put_irqrestore(&nklock,s);
	return wind_errnoget();
	}
...

{
    ...
    if (!__xn_access_ok(curr,VERIFY_WRITE,__xn_reg_arg2(regs),sizeof(wd->wdt)))
	return -EFAULT;
	...
	__xn_copy_to_user(curr,(void __user *)__xn_reg_arg2(regs),&wd->wdt,sizeof(wd->wdt));
...
static int __wind_int_context (struct task_struct *curr, struct pt_regs *regs)
{
    ...
    return pTcb && xnthread_base_priority(&pTcb->threadbase) == XNCORE_IRQ_PRIO;
}

static xnsysent_t __systab[] = {
    [__vxworks_task_init ] = { &__wind_task_init, __xn_exec_init },
    [__vxworks_task_activate ] = { &__wind_task_activate, __xn_exec_any },
    [__vxworks_task_deleteforce ] = { &__wind_task_deleteforce, __xn_exec_conforming },
    ...
    [__vxworks_task_priorityset ] = { &__wind_task_priorityset, __xn_exec_any },
    [__vxworks_task_priorityget ] = { &__wind_task_priorityget, __xn_exec_any },
    ...
    [__vxworks_kernel_timeslice ] = { &__wind_kernel_timeslice, __xn_exec_any },
