...
comedi_rngdesc_t range_bipolar10= 
    RNG_GLOBAL(rng_bipolar10);

comedi_rngtab_t rng_bipolar5={ 1, {
    RANGE_V(-5, 5),
}};
comedi_rngdesc_t range_bipolar5= 
    RNG_GLOBAL(rng_bipolar5);

comedi_rngtab_t rng_unipolar10={ 1, {
    RANGE_V(0, 10),
}};
comedi_rngdesc_t range_unipolar10= 
    RNG_GLOBAL(rng_unipolar10);

comedi_rngtab_t rng_unipolar5={ 1, {
    RANGE_V(0, 5),
}};
comedi_rngdesc_t range_unipolar5= 
    RNG_GLOBAL(rng_unipolar10);
			  ...
			  unsigned char nb_chan, unsigned int *chans)
{
  ...
  for(i=0;i<nb_chan;i++)
  {
    int j=(subd->rng_desc->mode!=COMEDI_RNG_GLOBAL_RNGDESC)?i:0;
    int k=(subd->chan_desc->mode!=COMEDI_CHAN_GLOBAL_CHANDESC)?i:0;

    if(CR_CHAN(chans[i])>=subd->chan_desc->length)
    {
      comedi_logerr("comedi_check_chanlist: chan idx out_of range (%u>=%u)\n",
		    CR_CHAN(chans[i]),subd->chan_desc->length);
      return -EINVAL;
    }
    if(CR_RNG(chans[i])>subd->rng_desc->rngtabs[j]->length)
    {
      comedi_logerr("comedi_check_chanlist: rng idx out_of range (%u>=%u)\n",
		  CR_RNG(chans[i]),subd->rng_desc->rngtabs[j]->length);
      return -EINVAL;
    }
    if(CR_AREF(chans[i])!=0 && 
       (CR_AREF(chans[i]) & subd->chan_desc->chans[k].flags)==0)
    {
      comedi_logerr("comedi_check_chanlist: bad channel type\n");
      return -EINVAL;
    }
  }
...
{  
    ...
    subd_info = comedi_kmalloc(dev->transfer->nb_subd*sizeof(comedi_sbinfo_t));
    if(subd_info == NULL)
	return -ENOMEM;
	...
	subd_info[i].nb_chan=dev->transfer->subds[i]->chan_desc->length;
    }
...
int comedi_ioctl_nbchaninfo(comedi_cxt_t *cxt, void *arg)
{
  ...
  if(comedi_copy_from_user(cxt, 
			   &inarg, arg, sizeof(comedi_chinfo_arg_t))!=0)
    return -EFAULT;

  if(inarg.idx_subd>=dev->transfer->nb_subd)
    return -EINVAL;

  inarg.info=(void *)(unsigned long)dev->transfer->subds[inarg.idx_subd]->chan_desc->length;
...
int comedi_ioctl_chaninfo(comedi_cxt_t *cxt, void *arg)
{
  ...
  chan_info = comedi_kmalloc(chan_desc->length *
			     sizeof(comedi_chinfo_t));
  if(chan_info==NULL)
    return -ENOMEM;

  for(i=0;i<chan_desc->length;i++)
  {
    int j = (chan_desc->mode!=COMEDI_CHAN_GLOBAL_CHANDESC)?i:0;
