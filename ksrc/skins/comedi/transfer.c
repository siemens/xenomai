...
int comedi_setup_transfer(comedi_cxt_t *cxt)
{
  ...
  for(i=0;i<tsf->nb_subd;i++)
  {
    if(tsf->subds[i]->flags & COMEDI_SUBD_CMD)
    {
      ...
      if(tsf->bufs[i] == NULL)
      {
	comedi_logerr("comedi_setup_transfer: call5-6(alloc) failed \n");
	ret=-ENOMEM;
	goto out_setup_tsf;
      }
...
int comedi_init_transfer(comedi_cxt_t *cxt, comedi_cmd_t *cmd)
{
	...
	set_bit(COMEDI_TSF_BULK, &(dev->transfer->status[cmd->idx_subd]));
	...
	for(i=0;i<cmd->nb_chan;i++) {
	    ...
	    chft = comedi_get_chfeat(dev->transfer->subds[cmd->idx_subd],
				     CR_CHAN(cmd->chan_descs[i]));
	    dev->transfer->bufs[cmd->idx_subd]->end_count+=chft->nb_bits/8;
	}
...
int comedi_cancel_transfer(comedi_cxt_t *cxt,int idx_subd)
{
	...
	comedi_logerr("comedi_cancel: subdevice %d cancel handler failed (ret=%d)\n",
		      idx_subd, ret);
    }
...
int comedi_ioctl_cancel(comedi_cxt_t *cxt, void *arg)
{
  ...
  if(dev->transfer->subds[idx_subd]->flags &
     COMEDI_SUBD_UNUSED)
    return -EIO;
