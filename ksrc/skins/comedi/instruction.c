...
int comedi_do_insn_trig(comedi_cxt_t *cxt, comedi_kinsn_t *dsc)
{
    ...
    if((subd->flags & COMEDI_SUBD_CMD) == 0 ||
       subd->trigger == NULL)
	return -EINVAL;
...
int comedi_fill_insndsc(comedi_cxt_t *cxt,
			comedi_kinsn_t *dsc, void *arg)
{
    int ret=0;
    void *tmp_data=NULL;
...
int comedi_fill_ilstdsc(comedi_cxt_t *cxt,
			comedi_kilst_t *dsc,void *arg)
{
    int i,ret=0;   
