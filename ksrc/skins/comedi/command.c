...
int comedi_fill_cmddesc(comedi_cxt_t *cxt, 
			comedi_cmd_t *desc, void *arg)
{
    int ret=0;
...
int comedi_check_cmddesc(comedi_cxt_t *cxt, comedi_cmd_t *desc)
{
	...
	comedi_logerr("comedi_check_cmddesc: subdevice index out of range (%u >= %u)\n",
		      desc->idx_subd,dev->transfer->nb_subd);
	return -EINVAL;
    }
  
    if(dev->transfer->subds[desc->idx_subd]->flags &
       COMEDI_SUBD_UNUSED) {
	comedi_logerr("comedi_check_cmddesc: subdevice type incoherent\n");
	return -EIO;
    }

    if(!(dev->transfer->subds[desc->idx_subd]->flags &
	 COMEDI_SUBD_CMD)) {
	comedi_logerr("comedi_check_cmddesc: operation not supported\n");
	return -EIO;
    }

    if(test_bit(COMEDI_TSF_BUSY, 
		&(dev->transfer->status[desc->idx_subd])))
	return -EBUSY;
...
int comedi_check_generic_cmdcnt(comedi_cmd_t *desc)
{
    ...
    tmp1 = desc->start_src & ~(TRIG_NOW|TRIG_INT|TRIG_EXT|TRIG_FOLLOW);
    ...
    if(desc->start_src != TRIG_NOW &&
       ...
       desc->start_src != TRIG_EXT &&
       desc->start_src != TRIG_FOLLOW)
	return -EINVAL;
    ...
    if(desc->convert_src != TRIG_TIMER &&
       desc->convert_src != TRIG_EXT &&
       desc->convert_src != TRIG_NOW)
	return -EINVAL;
  
    if(desc->stop_src!=TRIG_COUNT &&
       desc->stop_src!=TRIG_NONE)
	return -EINVAL; 
...
int comedi_check_specific_cmdcnt(comedi_cxt_t *cxt,comedi_cmd_t *desc)
{
	...
	dev->transfer->subds[desc->idx_subd]->cmd_mask;
...
int comedi_ioctl_cmd(comedi_cxt_t *cxt, void *arg)
{
	...
	ret = dev->transfer->subds[cmd_desc->idx_subd]->do_cmdtest(cxt,cmd_desc);
    if(ret != 0)
	goto out_ioctl_cmd;
    ...
    ret=dev->transfer->subds[cmd_desc->idx_subd]->do_cmd(cxt,cmd_desc->idx_subd);
    if(ret!=0) {
